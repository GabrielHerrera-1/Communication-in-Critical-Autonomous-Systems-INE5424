...
//
// vm1 RSU; vm2 subscriber (SmartData CONSUMIDOR de Unit::TEST_COUNTER);
// vm3..vm5 publishers PRODUTORES. Cada publisher e um componente que IMPLEMENTA
// IProducer. O SmartData observa o Communicator (que faz unmarshal e notifica
// com a Message inteira); o consumidor drena via receive_response().
...
public:
    ...
    void run() override {
        ...
        std::cout << "[" << LABEL << "][vm" << _vm_id << "] publisher PRODUTOR pronto" << std::endl;
...
public:
    ...
    void run() override {
        ...
        std::cout << "[" << LABEL << "][vm" << _vm_id
                  << "] subscriber CONSUMIDOR period_us=" << PERIOD_US << std::endl;
        ...
        while ((consumer.producer_count() < EXPECTED_PUBLISHERS ||
            ...
            Message * m = consumer.receive_response(2000); // drena a fila (timeout 2s)
            if (m) delete m;
        }
